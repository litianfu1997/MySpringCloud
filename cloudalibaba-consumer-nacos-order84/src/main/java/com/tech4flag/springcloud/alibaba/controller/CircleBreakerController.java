...
import com.tech4flag.springcloud.alibaba.service.PaymentService;
...
//    @SentinelResource(value = "fallback",blockHandler = "blockHandler") // 配置了blockHandler，只负责sentinel控制台配置违规
    ...
    public CommentResult<Payment> fallback(@PathVariable("id") Long id) {
        CommentResult<Payment> commonResult = restTemplate.getForObject(SERVICE_URL + "/paymentSQL/" + id, CommentResult.class);
        if (id == 4) {
    ...
    public CommentResult handlerFallback(Long id, Throwable e) {
        ...
        return new CommentResult(444, "兜底异常handler，exception内容" + e.getMessage(), payment);
    }

    public CommentResult blockHandler(Long id, BlockException exception) {
        Payment payment = new Payment(id, null);
        return new CommentResult<>(445, "blockHandler-sentinel 限流，无此流水号：blockException" + exception.getMessage(), payment);
    }
